...
# 安装 aimeshchat CLI + omp-mailbox-plugin + OMP 扩展配置。
#
# 用法：
...
#
# 可配置环境变量：
#   AIMESHCHAT_REF=v0.2.7         aimeshchat 安装 ref
#   MAILBOX_MIN_OK=1             跳过 aimeshchat 安装（已装则验证）
#   OMP_EXTENSION_SKIP=1         跳过 OMP extension 配置（仅装 CLI+plugin）
set -euo pipefail

AIMESHCHAT_REF="${AIMESHCHAT_REF:-v0.2.7}"
...
# ── 2. aimeshchat CLI（mailbox/aimeshchat 入口）─────────────────────────
if have aimeshchat && aimeshchat --version >/dev/null 2>&1; then
  say "aimeshchat already installed: $(aimeshchat --version)"
else
  say "installing aimeshchat@${AIMESHCHAT_REF} via uv tool install..."
  uv tool install "git+https://github.com/comicchang/aimeshchat.git@${AIMESHCHAT_REF}" --force
  have aimeshchat || die "aimeshchat install failed — add ~/.local/bin to PATH"
fi
