...
[aimeshchat](https://github.com/comicchang/aimeshchat) swarm/mailbox protocol.
...
```
Worker A:  aimeshchat swarm direct --session <id> --from A --to B ...  → <root>/<session>/B/inbox/{msg_id}.json
                              ↓ (SSH wire / 本地写入)
...
Thin notification adapter——所有 mailbox 操作委托给 aimeshchat 的 `mailbox`/`aimeshchat` CLI：

```bash
uv tool install git+https://github.com/comicchang/aimeshchat.git@v0.2.7
# 安装 'aimeshchat' + 'mailbox' + 'aimeshchat-remote-exec' 等入口
```

激活时做 capability check：`aimeshchat --version` ≥ `MAILBOX_MIN_VERSION`（0.1.0）；失败显式报错，不静默降级。
...
**prompt 引导 agent 定期 `aimeshchat mailbox peek` 轮询**同样可用（不依赖唤醒通知）。
...
跨主机：`aimeshchat mailbox ... --host <alias>`；高级 IPC：`aimeshchat swarm ...`。
...
- **launcher identity 必须非空**：`aimeshchat run --backend omp` 注入的 identity 若
